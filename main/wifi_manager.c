...
static void ip_event_handler(void* arg, esp_event_base_t event_base,
                            int32_t event_id, void* event_data);
static void update_wifi_state_unsafe(wifi_state_t state);
...
esp_err_t wifi_manager_connect(const char *ssid, const char *password)
{
    ...
    if (xSemaphoreTake(g_wifi_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
        ...
        update_wifi_state_unsafe(WIFI_STATE_CONNECTING);
    ...
    if (ret != ESP_OK) {
        ...
        if (xSemaphoreTake(g_wifi_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
            update_wifi_state_unsafe(WIFI_STATE_ERROR);
            xSemaphoreGive(g_wifi_mutex);
        }
        return ret;
    }
...
esp_err_t wifi_manager_disconnect(void)
{
    ...
    if (xSemaphoreTake(g_wifi_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
        update_wifi_state_unsafe(WIFI_STATE_DISCONNECTED);
        xSemaphoreGive(g_wifi_mutex);
    }
                            ...
                            int32_t event_id, void* event_data)
{
    if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ...
        if (xSemaphoreTake(g_wifi_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
            esp_ip4addr_ntoa(&event->ip_info.ip, g_wifi_info.ip_address, sizeof(g_wifi_info.ip_address));
            esp_ip4addr_ntoa(&event->ip_info.gw, g_wifi_info.gateway, sizeof(g_wifi_info.gateway));
            esp_ip4addr_ntoa(&event->ip_info.netmask, g_wifi_info.netmask, sizeof(g_wifi_info.netmask));
            ...
            update_wifi_state_unsafe(WIFI_STATE_CONNECTED);
            xSemaphoreGive(g_wifi_mutex);
        }
    }
}
